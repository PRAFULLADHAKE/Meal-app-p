// storing and retrieving data --its make a favourites meal array if its not exist in local storage
if (localStorage.getItem("favouritesList") == null) {
    localStorage.setItem("favouritesList", JSON.stringify([]));
}
// its fetch meals from api and return meals
...
function addRemoveToFavList(id) {
    ...
    if (contain) {
        ...
        arr.splice(number, 1);
        alert("Do you want to remove meal from favourites list");
    } else {
        arr.push(id);
        alert("Do you want to add meal to favourites list");
    }
